...
from app.domain.models.user_data import History, Stock, UserPortfolio, UsersInfo
...
class JSONUserDataRepository(UserDataRepositoryPort):
    ...
    def delete_user(self, user_id:str):
        users_info = self._get_user_info_from_json()
        users_info.users_info = [user for user in users_info.users_info if user.name != user_id ]
        self._update_user_info(users_info=users_info)
        return 
    ...
    def patch_user(self, user_to_update: UserPortfolio):
        ...
        for user_info in users_info.users_info:
            ...
            user_found = False
            if user_info.name == user_to_update.name:
                user_found = True
                # Original stock list 
